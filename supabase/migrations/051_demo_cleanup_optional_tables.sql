...
begin
  ...
  if to_regclass('public.invoice_drafts') is not null then
  ...
  if to_regclass('public.facilitator_event_reminders') is not null then
  ...
  if to_regclass('public.event_exposure_stats') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.event_main_categories') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.facilitator_event_reference_counters') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.facilitator_event_reminder_notifications') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.facilitator_profile_views') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.facilitator_service_titles') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.facilitator_tags') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.homepage_event_collection_events') is not null then
    insert into demo_cleanup_counts values
  ...
  if to_regclass('public.invoice_draft_lines') is not null then
    insert into demo_cleanup_counts values
...
comment on function public.cleanup_demo_data(uuid[], text[], text[], boolean) is
  'Admin-only cleanup for explicitly selected demo facilitators. Dry run by default. Skips optional tables that do not exist. Does not delete auth.users or public.profiles.';
