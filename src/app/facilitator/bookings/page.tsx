...
export default async function FacilitatorBookingsPage({ searchParams }: FacilitatorBookingsPageProps) {
  ...
  const { data: eventOptions } = facilitatorProfile
    ? await supabase
        .from("events")
        .select("id, title, starts_at, ends_at, status, capacity, address_line, city, cover_image_path, price_cents, registration_mode, bookings(id, status, seats, booking_value_cents), event_payment_settings(method_source, payment_link_mode), event_main_categories(main_categories(name, color_hex, image_path))")
