...
export default async function FacilitatorOpenGraphImage({ params }: OpenGraphImageProps) {
  ...
  const rows = await fetchOpenGraphRows<FacilitatorOpenGraphRow>(
    "facilitator_profiles?select=company_name,profile_image_path,short_description,long_description,profiles!facilitator_profiles_profile_id_fkey(full_name)&" +
      lookupColumn +
      "=eq." +
  ...
  const imageUrl = storagePublicUrl(facilitator.profile_image_path) ?? (await getHomepageOpenGraphImageUrl());
